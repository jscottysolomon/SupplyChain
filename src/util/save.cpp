...
// void to_json(nlohmann::json& j, const Truck& truck) {
//   j = nlohmann::json{
//       {"entityId", truck.GetId()},
//       {"junctionId", truck.GetJunction()->GetId()},
//       {"roadSegmentId", truck.GetRoadSegment()->GetId()},
//       {"isOnRightLane", truck.IsOnRightLane()},
//       {"position", {
//           {"x", truck.GetPosition().x},
//           {"y", truck.GetPosition().y}
//       }},
//       {"schedule", SerializeList(truck.GetSchedule())},
//       {"stops", SerializeList(truck.GetStops())},
//       {"inventory", truck.GetInventory()}
//   };
// }
...
void WriteSave(TrafficCommand& commander) {
  ...
  commander.ForEachJunction([&](int id, Junction& junc) {
    ...
    if (auto* typed = dynamic_cast<Factory*>(node)) {
